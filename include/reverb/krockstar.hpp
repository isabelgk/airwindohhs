...
template <typename T>
class kRockstar final : public Effect<T>
{
    ...
    static constexpr unsigned long kUniqueId = 'krok';
    float A;
    float B;
    float C;
    float D;
    ...
    void process(T** inputs, T** outputs, long sampleFrames)
    {
        ...
        while (--sampleFrames >= 0)
        {
            ...
            if (bez[bez_cycle] > bezTrim) { // hit the end point and we do a reverb sample
                ...
                if (roomTimerR > 4) {
                    roomTimerR = 0;
                    if (roomNoiseR > fmin(reg6n + (applyNonlin * 0.0002), regenMax)) {
                        roomNoiseR -= fabs((fpdR / (double)UINT32_MAX) - 0.5) * 0.000007629394531;
                    }
                    else {
                        roomNoiseR += fabs((fpdR / (double)UINT32_MAX) - 0.5) * 0.000007629394531;
                    }
                    ...
                    roomNoiseR += ((fpdR / (double)UINT32_MAX) - 0.5) * 0.000007629394531;
                }
