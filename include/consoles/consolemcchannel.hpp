...
template <typename T>
class ConsoleMCChannel final : public Effect<T>
{
    double avgAL;
    double avgAR;
    double avgBL;
  ...
  public:
    ...
    void process(T** inputs, T** outputs, long sampleFrames)
    {
        ...
        while (--sampleFrames >= 0)
        {
            ...
            if (overallscale > 1.1) {
                avg = (trebleL + avgAL) * 0.5;
                avgAL = trebleL;
                trebleL = avg;
                avg = (trebleR + avgAR) * 0.5;
                avgAR = trebleR;
                trebleR = avg;
                if (overallscale > 2.1) {
                    avg = (trebleL + avgBL) * 0.5;
                    avgBL = trebleL;
                    trebleL = avg;
                    avg = (trebleR + avgBR) * 0.5;
                    avgBR = trebleR;
                    trebleR = avg;
                }
            }
            ...
            if (overallscale > 1.1) {
                avg = (trebleL + avgCL) * 0.5;
                avgCL = trebleL;
                trebleL = avg;
                avg = (trebleR + avgCR) * 0.5;
                avgCR = trebleR;
                trebleR = avg;
            }
            // begin ResEQ2 Mid Boost
