...
constexpr std::string_view k_name{ "Cabs" };
constexpr std::string_view k_short_description{
    "Cabs is an Airwindows guitar speaker cabinet simulator."
};
constexpr std::string_view k_long_description{
    "I’ve been asked for ITB guitar plugins before. Never been all that interested, because I like real amps and real tubes etc: have never found a lot of enthusiasm for going to great lengths to digitally fake all the colorations of some amp/speaker or other, because when the soul’s not there the color is irrelevant.This is different. I took on the challenge of updating my old Cabs plugin: originally dynamically convolved cab impulses, except they hadn’t been MADE with that in mind so it became a series of wild experiments. I wasn’t happy… then.Cabs completely overhauled everything I’d attempted. It’s undersampled, so it’ll run on whatever sample rate you like, efficiently. It’s got the same tricks of allowing you to heighten the particular speaker coloration it uses, or dial it right back to nothing.But around that core is a completely different approach to getting ITB amp tones. I’m using the cab size and ‘room loud’ controls to let you dial in the exact presentation you want, from clear and up front power-soaked clearness to complete wall-of-stacks meltdown, and beyond (in classic Airwindows fashion). This is done through an alternate approach to loudness emulation based on the destruction of audio signals through ultimate volume, such as a Space Shuttle takeoff, and what happens to the waveforms there.I think you should listen to it. Careful that you don’t jump for odd settings right away and get confused: for instance, settings of Room Loud beyond say 0.6 are not really real-world things. Settings of the cab tone intensity beyond 0.6 or 0.7 might be a bit weird. The way the cabs take on character and intensity is related to cab size: everything is basically related to everything else. You have to know what sound you’re trying to get, to clone something real-world. This is not a preset box, at all, at all. You have to be a real guitar tone maven to dial this in to convincing amp clone territory.OR… or… or!You can use the range of adjustment here, with the controls that give you a basic idea of ‘what you’re doing and how you want it’, combined with for instance Edge or the upcoming Dirt plugin (softer saturation) to get tones that DO NOT EXIST but act like real amps in significant ways. You can get tones that you can’t get in the real world, and dial them in to match what’s in your head: in some cases maybe to a point that the real world amps couldn’t reach.And track through them because the whole rig (when using entirely Airwindows plugins) runs NO LATENCY, so it’ll feel as close as it can to the real thing, and respond with electric immediacy.And then unlike any real amp situation… once you’ve got the final mix done, you can wiggle any of these parameters to where they need to be for THE SONG and tune the tones to best suit whatever else is there, even if it didn’t exist when you were tracking it. This is a freedom of the DAW experience (don’t go too overboard, make decisions!) and it might bring you the results you need.And all of this can be lightly applied to any other tone: real genuine amp-and-cab that was tracked too quietly or didn’t pick up enough mojo, synths, drums, you name it!"
};
...
template <typename T>
class Cabs final : public Effect<T>
{
    ...
    void process(T** inputs, T** outputs, long sampleFrames)
    {
        ...
        while (--sampleFrames >= 0)
        {
            ...
            if (cycle == cycleEnd) { // hit the end point and we do a chorus sample
                ...
                if (-clamp > rarefaction) {
                    inputSampleL = lastSampleL - rarefaction;
                }
                ...
                if (-clamp > rarefaction) {
                    inputSampleR = lastSampleR - rarefaction;
                }
                ...
                if (clamp > threshold) {
                    ataHalfwaySampleL = lastHalfSampleL + threshold;
                }
                if (-clamp > rarefaction) {
                    ataHalfwaySampleL = lastHalfSampleL - rarefaction;
                }
                lastHalfSampleL = ataHalfwaySampleL; //-------------------------------------------
                clamp = ataHalfwaySampleR - lastHalfSampleR;
                if (clamp > threshold) {
                    ataHalfwaySampleR = lastHalfSampleR + threshold;
                }
                if (-clamp > rarefaction) {
                    ataHalfwaySampleR = lastHalfSampleR - rarefaction;
                }
                ...
                if (clamp > threshold) {
                    inputSampleL = lastPostSampleL + threshold;
                }
                if (-clamp > rarefaction) {
                    inputSampleL = lastPostSampleL - rarefaction;
                }
                lastPostSampleL = inputSampleL; //-------------------------------------
                clamp = inputSampleR - lastPostSampleR;
                if (clamp > threshold) {
                    inputSampleR = lastPostSampleR + threshold;
                }
                if (-clamp > rarefaction) {
                    inputSampleR = lastPostSampleR - rarefaction;
                }
                ...
                if (clamp > threshold) {
                    ataHalfwaySampleL = lastPostHalfSampleL + threshold;
                }
                if (-clamp > rarefaction) {
                    ataHalfwaySampleL = lastPostHalfSampleL - rarefaction;
                }
                lastPostHalfSampleL = ataHalfwaySampleL; //----------------------------------
                clamp = ataHalfwaySampleR - lastPostHalfSampleR;
                if (clamp > threshold) {
                    ataHalfwaySampleR = lastPostHalfSampleR + threshold;
                }
                if (-clamp > rarefaction) {
                    ataHalfwaySampleR = lastPostHalfSampleR - rarefaction;
                }
                ...
                if (flip)
                {
                    ...
                    if (iirHalfHeadBumpAL > 0) {
                        iirHalfHeadBumpAL -= dcblock;
                    }
                    if (iirHalfHeadBumpAL < 0) {
                        iirHalfHeadBumpAL += dcblock;
                    }
                    ...
                    if (iirHalfHeadBumpAR > 0) {
                        iirHalfHeadBumpAR -= dcblock;
                    }
                    if (iirHalfHeadBumpAR < 0) {
                        iirHalfHeadBumpAR += dcblock;
                    }
                ...
                else
                {
                    ...
                    if (iirHalfHeadBumpBL > 0) {
                        iirHalfHeadBumpBL -= dcblock;
                    }
                    if (iirHalfHeadBumpBL < 0) {
                        iirHalfHeadBumpBL += dcblock;
                    }
                    ...
                    if (iirHalfHeadBumpBR > 0) {
                        iirHalfHeadBumpBR -= dcblock;
                    }
                    if (iirHalfHeadBumpBR < 0) {
                        iirHalfHeadBumpBR += dcblock;
                    }
                ...
                inputSampleL += ((ataDiffSampleL + ataHalfDiffSampleL + ataPrevDiffSampleL) / 1.0);
                ataPrevDiffSampleL = ataDiffSampleL / 2.0; //----------------------------
                inputSampleR = ataDrySampleR;
                inputSampleR += ((ataDiffSampleR + ataHalfDiffSampleR + ataPrevDiffSampleR) / 1.0);
                ...
                if (clamp > postThreshold) {
                    inputSampleL = postPostSampleL + postThreshold;
                }
                if (-clamp > postRarefaction) {
                    inputSampleL = postPostSampleL - postRarefaction;
                }
                ...
                if (clamp > postThreshold) {
                    inputSampleR = postPostSampleR + postThreshold;
                }
                if (-clamp > postRarefaction) {
                    inputSampleR = postPostSampleR - postRarefaction;
                }
