...
template <typename T>
class Monitoring2 final : public Effect<T>
{
    enum
    {
        fix_freq,
...
void process(T** inputs, T** outputs, long sampleFrames)
{
    ...
    int allpasstemp;
    // for PeaksOnly
    biquad[fix_freq] = 0.0375 / overallscale;
    biquad[fix_reso] = 0.1575; // define as AURAT, MONORAT, MONOLAT unless overridden
    if (processing == kVINYL) {
        biquad[fix_freq] = 0.0385 / overallscale;
        biquad[fix_reso] = 0.0825;
    }
    if (processing == kPHONE) {
        biquad[fix_freq] = 0.1245 / overallscale;
        biquad[fix_reso] = 0.46;
    }
    ...
    while (--sampleFrames >= 0)
    {
        ...
        switch (processing)
        {
            ...
            case kPHONE:
                ...
                if (processing == kMONORAT) {
                    inputSampleR = (inputSampleL + inputSampleR) * 0.5;
                    inputSampleL = 0.0;
                }
                if (processing == kMONOLAT) {
                ...
                double tempSampleL;
                tempSampleL = (inputSampleL * biquad[fix_a0]) + biquad[fix_sL1];
                ...
                double tempSampleR;
                tempSampleR = (inputSampleR * biquad[fix_a0]) + biquad[fix_sR1];
            ...
            case kCANSC:
            case kCANSD:
                if (processing == kCANSA) {
                    inputSampleL *= 0.855;
                    inputSampleR *= 0.855;
                }
                if (processing == kCANSB) {
                    inputSampleL *= 0.748;
                    inputSampleR *= 0.748;
                }
                if (processing == kCANSC) {
                    inputSampleL *= 0.713;
                    inputSampleR *= 0.713;
                }
                if (processing == kCANSD) {
                    inputSampleL *= 0.680;
                    inputSampleR *= 0.680;
                }
