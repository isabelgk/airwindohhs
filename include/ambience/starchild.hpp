...
template <typename T>
class StarChild final : public Effect<T>
{
    uint32_t fpdL;
    uint32_t fpdR;
    // default stuff
  ...
  public:
        ...
        for (count = 0; count < 165; count++)
        {
            ...
            switch (assign) {
                case 1:
                    ...
                    break;
                case 3:
                    outL[count] = p[count] * 0.25;
                    outR[count] = p[count] * 0.75;
                    break;
                case 7:
                    outL[count] = p[count] * 0.75;
                    outR[count] = p[count] * 0.25;
                    break;
                case 9:
                    outL[count] = p[count];
                    outR[count] = 0.0;
    ...
    constexpr std::string_view get_parameter_name(int index)
    {
        switch (static_cast<params>(index))
        {
            case kParamA: return "sustain"; break;
            case kParamB: return "grain"; break;
    ...
    void process(T** inputs, T** outputs, long sampleFrames)
    {
        ...
        while (--sampleFrames >= 0)
        {
            ...
            wearR[2] = wearR[1];
            wearR[1] = wearR[0];
            wearR[0] = accumulatorSample = (inputSampleR - wearRPrev);
            ...
            wearL[2] = wearL[1];
            wearL[1] = wearL[0];
            wearL[0] = accumulatorSample = (inputSampleL - wearLPrev);
            accumulatorSample *= factor[0];
