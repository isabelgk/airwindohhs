...
if __name__ == "__main__":
    root_dir = "/Users/igk/code/igk/libairwindows"
    template_file = Path(os.path.join(root_dir, "scripts", "template.hpp"))
    dstdir = Path(os.path.join(root_dir, "include"))
    categories = get_category_lines("/Users/igk/code/igk/libairwindows/docs/airwindopedia.txt")
    for category, names in categories:
        for name in names:
            srcdir = Path(os.path.join(root_dir, "tmp", "air", name))
            try:
